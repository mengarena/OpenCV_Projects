...
SurfMatching::SurfMatching(void):ratio(0.8f),distance(1.0),confidence(0.98),refineF(true)
{
}
...
void SurfMatching::setRatio(float rt)
{
	ratio = rt;
}


void SurfMatching::setDistance(double dst)
{
	distance = dst;
}


void SurfMatching::setConfidence(double confd)
{
	confidence = confd;
}
...
int SurfMatching::ratioTest(vector<vector<DMatch>> &matches)
{
	int removed = 0;
...
void SurfMatching::symmetryTest(const vector<vector<DMatch>>& matches1,const vector<vector<DMatch>>& matches2, vector<DMatch>& symMatches)
{
...
void SurfMatching::ransacTest(const vector<DMatch>& matches, const vector<KeyPoint>& keypoints1, const vector<KeyPoint>& keypoints2, vector<DMatch>& outMatches)
{
...
void SurfMatching::filterWithSlopeDistance(	Mat& image1, 
											Mat& image2, 
											...
											vector<DMatch>& finalMatches )
{
	...
	//cout << "fAngle= " <<fAngle <<", x1=" <<x1 <<", y1="<<y1 <<", x2=" <<(x2-width1) << ",y2=" <<y2 << endl;
	...
	for (; itAngleV != linesAngle.end(); itAngleV++, itEuDistV++, itM++) {
		...
		if ((fCurrentAngle >= minAngle) && (fCurrentAngle <= maxAngle) && (fCurrentEuDist >= minEuDist) && (fCurrentEuDist <= maxEuDist)) {
			...
			//cout <<  "#####Angle: " << fCurrentAngle << endl;
			//cout <<  "#####Distance: " << fCurrentEuDist << endl;
...
//vector<DMatch> SurfMatching::DoMatching(Mat& image1, Mat& image2, double &time )

vector<DMatch> SurfMatching::DoMatching(Mat& image1, Mat& image2, int nFeatureNum, double &time, int &nNumKeypoint1, int &nNumKeypoint2, double &featureSize1, double &featureSize2)
//vector<DMatch> SurfMatching::DoMatching(Mat &image1, Mat &image2, double &time, float &fMaxFeatureRatio, float &fAvgFeatureRatio)
{
	...
	//SURF method
//	SurfFeatureDetector surf(400);  //minHessian 
//	SurfFeatureDetector surf(5., 4, 5); //All default parameters
//###	SurfFeatureDetector detector(5., 8, 5); //All default parameters

//	FastFeatureDetector surf; //All default parameters
...
//###	SurfDescriptorExtractor extractor;
	...
	//Extract SURF feature points/feature descriptors for image 1
...
////	surfDesc.compute(image1, keypoints1, descriptors1);
	...
	cout<<"...................................."<<endl;
	//cout<<"Ln==="<<strlen((unsigned char*)(descriptors1.data))<<endl;
...
//	}
	...
	OrbFeatureDetector detector(nFeatureNum, 1.1, 10); 
///	OrbFeatureDetector detector(1000, 1.2, 5);
//	OrbFeatureDetector detector(nFeatureNum); 
...
#if 0
		char *ret11 = new char[100];

		//open the file to write the resultant descriptor
		sprintf(ret11, "F:\\tmp\\keypoint11.txt");
		FileStorage fs1(ret11, FileStorage::WRITE);    
		fs1 << keypoints1;        
		fs1.release();
#endif
...
//	cout<<"TotalSize="<<featureSize1<<endl;

#if 0
	//Write into file f:\\Fp.txt
	ofstream myfile;
	myfile.open ("F:\\forImageFeatureSize\\Fp.txt", ios::out | ios::binary);
	//unsigned char *temp = descriptors1.data;
	myfile.write(reinterpret_cast<char*>(descriptors1.data), descriptors1.elemSize()* matSize.width * matSize.height);
	//for (int kk1 = 0; kk1< descriptors1.rows; kk1++) 
	//	for (int kk2 = 0; kk2< descriptors1.cols; kk2++)
...
#if 0
		for(vector<DMatch>::const_iterator it = matches.begin(); it != matches.end(); ++it) {
			cout<<"haha="<<keypoints1[it->queryIdx].octave<<"	,huhu="<<keypoints2[it->trainIdx].octave<<endl;
...
#if 0
vector<DMatch> SurfMatching::DoMatching(Mat& image1, Mat& image2)
{
	vector<KeyPoint> keypoints1, keypoints2;
	Mat descriptors1, descriptors2;
...
void SurfMatching::ransacTest_Special(const vector<DMatch>& matches, const vector<Point2f>& points1, const vector<Point2f>& points2, vector<DMatch>& outMatches)
{
	//Convert keypoints into Point2f
	vector<Point2f> points11,points22;

	for(vector<DMatch>::const_iterator it = matches.begin(); it != matches.end(); ++it) {
		//Get the position of left keypoints
		float x = points1[it->queryIdx].x;
		float y = points1[it->queryIdx].y;
		points11.push_back(Point2f(x,y));
		//Get the position of right keypoints
		x = points2[it->trainIdx].x;
		y = points2[it->trainIdx].y;
		points22.push_back(Point2f(x,y));
	}

	//Computer Fundamental matrix using RANSAC
	vector<uchar> inliers(points11.size(), 0);
	Mat fundamental = findFundamentalMat(Mat(points11), Mat(points22), inliers, CV_FM_RANSAC, distance, confidence);
...
void SurfMatching::filterWithSlopeDistance_Special(int widthBase, 
											...
											vector<DMatch>& finalMatches )
{
	...
	for(vector<DMatch>::const_iterator it = inMatches.begin(); it != inMatches.end(); ++it) {
		//Get the position of left keypoints
		float x1 = points1[it->queryIdx].x;
		float y1 = points1[it->queryIdx].y;

		//Get the position of right keypoints
		float x2 = points2[it->trainIdx].x + widthBase;
		float y2 = points2[it->trainIdx].y;

		float fEuDist = sqrt( pow((x2 - x1),2)*1.0 + pow((y2 - y1), 2) );
		float fAngle = atan( (y2 - y1)*1.0 /(x2 - x1) )*180/pi; //Get Degree
 //   cout << "fAngle= " <<fAngle <<", y2=" <<y2 <<", y1="<<y1 <<", x2=" <<x2 << ",x1=" <<x1 << endl;
...
//vector<DMatch> SurfMatching::DoMatching_Special(Mat& image1, Mat& image2, vector<KeyPoint>& keypoints1, vector<KeyPoint>& keypoints2, Mat& descriptors1, Mat& descriptors2, vector<Point2f> points1, vector<Point2f> points2, int widthBase, double &time)

vector<DMatch> SurfMatching::DoMatching_Special(Mat& image1, Mat& image2, vector<KeyPoint>& keypoints1, vector<KeyPoint>& keypoints2, Mat& descriptors1, Mat& descriptors2, int widthBase, double &time)
{
...
//	FlannBasedMatcher matcher;
	...
	//Validate matches using RANSAC
	ransacTest(symMatches, keypoints1, keypoints2, matches);
////	ransacTest_Special(symMatches, points1, points2, matches);

	cout<<"ransacMatches===   "<<matches.size()<<endl;
	if (matches.size() < 5) {
		//matches.clear();
		////////////////////###return matches;
	}
	...
	filterWithSlopeDistance(image1, image2, keypoints1, keypoints2, matches, finalMatches);
////	filterWithSlopeDistance_Special(widthBase, points1, points2, matches, finalMatches);
	cout<<"finalMatches===  "<<finalMatches.size() <<endl; 
	...
	drawMatches(image1, keypoints1, image2, keypoints2, finalMatches, imageMatchesFinal, Scalar(0,0,255));
//	drawMatches(image1, keypoints1, image2, keypoints2, symMatches, imageMatchesFinal, Scalar(0,0,255));
//	drawMatches(image1, keypoints1, image2, keypoints2, matches, imageMatchesFinal, Scalar(0,0,255));
